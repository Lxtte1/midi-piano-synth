...
Piano::Piano(AudioManager& audioManager, QWidget* parent) : QWidget(parent) {
    ...
    for (unsigned int i = 0; i < NUM_KEYS; i++) {
        ...
        QLabel* label = new QLabel(QString("%1\%2%3").arg(KEYS[i % 12]).arg(KEY_COLOUR[i % 12] ? "#" : "").arg(START_OCTAVE + (int)(i / 12)));
